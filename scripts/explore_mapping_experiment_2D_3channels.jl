...
more specifically from a 3-readout-channel sensor.
A facet plot is created with the rows and columns of the
facet corresponding to a 2D histogram. Additionally,
the heatmap of counts from each individual readout is
created.
"""
using DrWatson
...
# configuration of the `experiment`'s counts heatmap
experiment_heatmap_counts_config = HeatmapCountsExperimentConfig(
    :ϕ,
    :θ;
...
# construct the heatmap of counts for the 3-channel sensor
hm_experiment = construct_experiment_counts_heatmap(experiment, experiment_heatmap_counts_config)
resize_to_layout!(hm_experiment.fig)
